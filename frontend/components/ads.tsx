  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-4 py-3 text-sm text-center">
          <span
            className={`px-2 py-1 rounded text-xs font-medium ${getAdStatusColor(ad.status)}`}
          >
      ...
      {isExpanded && (
        <tr className="bg-indigo-50 border-b border-gray-200">
          <td colSpan={8} className="px-4 py-4">
            <div className="font-semibold text-indigo-900 mb-3">
              ✈ Aircraft Status
            </div>
            {statusData.length === 0 ? (
              <div className="text-center text-gray-500 text-sm py-2">
                No flights assigned to this ad
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {(ad.flights || []).map((flight, idx) => {
                  ...
                  const issueBadge = status.issue?.hasIssue ? (
                    <span className="px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-900">
                      ISSUE
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-900">
                      OK
                    </span>
                  );
                  ...
                  return (
                    <div
                      key={flight}
                      className="p-2 bg-white border border-indigo-200 rounded text-sm"
                    >
                      ...
                      <div className="text-xs text-gray-600 mt-1">
                        {status.issue?.message || "No issues"}
                      </div>
                    </div>
                  );
                })}
